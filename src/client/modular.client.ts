...
print("Modular Client is running");
...
// Set local client state (now type-safe)
framework.SetState<number>("localScore", 100);
framework.SetState<string>("playerName", "TestPlayer");

// Subscribe to local state changes (type-safe callbacks)
...
// Subscribe to server state changes (type-safe)
